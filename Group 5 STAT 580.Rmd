...
pos_words <- scan("~/Desktop/STAT 580 Project/positive-words.txt", 
                  what = "character")
neg_words <- scan("~/Desktop/STAT 580 Project/negative-words.txt", 
                  what = "character")
...
tempCheck <- function(data){
  ...
  test_score <- sapply(clean_test, getMyScore)
  tweets_hist <- plot_ly(x = ~test_score, type = "histogram",
                         marker = list(color = fifaPalette[4],
                                       line = list(color = fifaPalette[2],
                                                   width = 2))) %>% 
    layout(title = "Frequency distribution of sentiment scores",
